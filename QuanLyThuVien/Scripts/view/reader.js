...
class Reader {
    ...
    constructor() {
        ...
        //Chọn các button trong sự kiện contextMenu
        this.setClickContextMenu();
        //Context menu
        this.setContextMenu();
        //Close Context Menu
    ...
    //hàm gọi đến sự kiện chuột trái
    setClickContextMenu() {
        //Hàm khởi tạo với sự kiện chuột trái Context menu
        // Nếu bấm vào icon thêm mới 
        $('#btn-Add-Reader').click(this.RegistrationOnClick);
        // Nếu bấm vào icon thêm mới vào danh sách học sinh
        $('#btn-Add-List').click(this.RegistrationListOnClick);
        // Nếu bấm vào icon sửa
        $('#btn-Edit').click(this.BindDataOnDialogEdit);
        // nếu bấm vào icon xóa chuột phải
        $('#btn-Delete').click(this.ClickButtonDelete);
        // nếu bấm vào icon nạp
        $('#btn-Refresh').click(this.ReloadTable);
        // Nếu chọn icon giúp 
        $('#btn-Help').click();
    }
    //Hàm xử lý chức năng trên sự kiện context menu 
    ...
    setTabIndex() {
        ...
        $('#focus1').keyup(function (e) {
            if (e.keyCode === 9) $("#txtLastName").focus();
        });
        //Shift tab
